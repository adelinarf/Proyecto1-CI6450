...
const SCORE_TO_GET_TO_LAST_ROW = 380
...
func _ready() -> void:
	createLines = Show.show
	can_die = not createLines
	changeWallsColor()	
	...
	g1 = Graph.new()
	t = g1.visibilityGraphEasiest(all_vertexes,vertexes,$".",createLines)
	...
	character12 = get_node("Character12")
	character13 = get_node("Character13")
	character14 = get_node("Character14")
	character15 = get_node("Character15")
	character16 = get_node("Character16")
	...
	character12.disableArrows()
	character12.disableCollisions()
	character13.disableArrows()
	character13.disableCollisions()
	character14.disableArrows()
	character14.disableCollisions()
	character15.disableArrows()
	character15.disableCollisions()
	character16.disableArrows()
	character16.disableCollisions()
	...
	wander6 = Wander.new(character6,target)
	arrive6 = Arrive.new(character6,target)
	evade6 = Evade.new(character6,target)
	flee6 = Flee.new(character6,target)
	
	seek7 = Seek.new(character7,target)
	attack7 = Arrive.new(character7,target)
	wander7 = Wander.new(character7,target)
	arrive7 = Arrive.new(character7,target)
	evade7 = Evade.new(character7,target)
	flee7 = Flee.new(character7,target)
	
	seek8 = Seek.new(character8,target)
	attack8 = Arrive.new(character8,target)
	wander8 = Wander.new(character8,target)
	arrive8 = Arrive.new(character8,target)
	evade8 = Evade.new(character8,target)
	flee8 = Flee.new(character8,target)
	
	seek9 = Seek.new(character9,target)
	attack9 = Arrive.new(character9,target)
	wander9 = Wander.new(character9,character10)
	arrive9 = Arrive.new(character9,target)
	evade9 = Evade.new(character9,target)
	flee9 = Flee.new(character9,target)
	
	seek10 = Seek.new(character10,target)
	attack10 = Arrive.new(character10,target)
	wander10 = Wander.new(character10,target)
	arrive10 = Arrive.new(character10,target)
	evade10 = Evade.new(character10,target)
	flee10 = Flee.new(character10,target)
	
	seek11 = Seek.new(character11,target)
	attack11 = Arrive.new(character11,target)
	wander11 = Wander.new(character11,target)
	arrive11 = Arrive.new(character11,target)
	evade11 = Evade.new(character11,target)
	flee11 = Flee.new(character11,target)
	
	seek12 = Seek.new(character12,target)
	attack12 = Arrive.new(character12,target)
	wander12 = Wander.new(character12,target)
	arrive12 = Arrive.new(character12,target)
	evade12 = Evade.new(character12,target)
	flee12 = Flee.new(character12,target)
	
	seek13 = Seek.new(character13,target)
	attack13 = Arrive.new(character13,target)
	wander13 = Wander.new(character13,target)
	arrive13 = Arrive.new(character13,target)
	evade13 = Evade.new(character13,target)
	flee13 = Flee.new(character13,target)
	
	seek14 = Seek.new(character14,target)
	attack14 = Arrive.new(character14,target)
	wander14 = Wander.new(character14,target)
	arrive14 = Arrive.new(character14,target)
	evade14 = Evade.new(character14,target)
	flee14 = Flee.new(character14,target)
	
	seek15 = Seek.new(character15,target)
	attack15 = Arrive.new(character15,target)
	wander15 = Wander.new(character15,target)
	arrive15 = Arrive.new(character15,target)
	evade15 = Evade.new(character15,target)
	flee15 = Flee.new(character15,target)
	
	seek16 = Seek.new(character16,target)
	attack16 = Arrive.new(character16,target)
	wander16 = Wander.new(character16,target)
	arrive16 = Arrive.new(character16,target)
	evade16 = Evade.new(character16,target)
	flee16 = Flee.new(character16,target)
		
	old_modulation = character.modulate
	#point,restore,bonus
	var points = getNodes("point",1,95)
	for point in points:
		point.play("move_coin")
	var restore = getNodes("restore",1,17)
...
var color_char1 = Color.CHARTREUSE
var color_char2 = Color.DARK_SLATE_BLUE
var color_char3 = Color.MISTY_ROSE
var color_char4 = Color.ORANGE
var color_char5 = Color.OLIVE
var color_char6 = Color.MEDIUM_PURPLE


var decision_colors = [Color.RED,Color.BLUE,Color.GREEN,Color.YELLOW,Color.WEB_PURPLE,color_char1,color_char2,color_char3,color_char4,color_char5,color_char6]
...
func detectCollisionPlayer():
	...
	if col.position != Vector2.ZERO:
		l._open()
		won = true
		$Player/open_chest.play()
	
	var ch = getNodes("Character",1,16)
	...
	if collision3 != null:
		...
		if "restore" in collision3.name:
			target.recover(20)
			$Player/restore_sound.play()
			remove_child(collision3)
		if "bonus" in collision3.name:
			target.recover(50)
			$Player/bonus_sound.play()
			remove_child(collision3)
		if "spike" in collision3.name:
			$Player/spike_sound.play()
			target.velocity = Vector2.ZERO
...
func detectCollisionCharacter(char):
	...
	if collision.position != Vector2.ZERO:
		var orientation_vec = Vector2.from_angle(char.orientation).normalized()
		char.velocity =( char.velocity.orthogonal() + 2*orientation_vec)*2
...
var can_die = false
var selected_colors = []
var running = [1,2,3,4,5,6,7]
var createLines = false
func _process(delta: float) -> void:
	...
	if won:
		...
		$smoke.visible = true
		$smoke.play("move")
		await $Player/open_chest.finished
		...
		if not createLines:
			await $Player/game_over_sound.finished	
		Show.game_over = true
		get_tree().paused = true
		show()
		$Player/menu.visible = true
		$Player/menu.game_over()
	else:		
		...
		if 6 in running:
			decision_first(character6,1,seek6,attack6,wander6,arrive6,evade6,flee6,color_char1)
			decision_first(character7,2,seek7,attack7,wander7,arrive7,evade7,flee7,color_char2)
			var res8 = decision_first(character8,3,seek8,attack8,wander8,arrive8,evade8,flee8,color_char3,counter3,hammers3)
			counter3 = res8[0]
			hammers3 = res8[1]
			hammers3 = check_hammer_collision(hammers3,character8)
			decision_first(character9,4,seek9,attack9,wander9,arrive9,evade9,flee9,color_char4)
			decision_first(character10,5,seek10,attack10,wander10,arrive10,evade10,flee10,color_char5)
			var res11 = decision_first(character11,6,seek11,attack11,wander11,arrive11,evade11,flee11,color_char6,counter4,hammers4)
			counter4 = res11[0]
			hammers4 = res11[1]
			hammers4 = check_hammer_collision(hammers4,character11)
			
		if 7 in running:
			decision_first(character12,1,seek12,attack12,wander12,arrive12,evade12,flee12,color_char1)
			decision_first(character14,1,seek14,attack14,wander14,arrive14,evade14,flee14,color_char1)
			
			var res13 = decision_first(character13,3,seek13,attack13,wander13,arrive13,evade13,flee13,color_char3,counter5,hammers5)
			counter5 = res13[0]
			hammers5 = res13[1]
			hammers5 = check_hammer_collision(hammers5,character13)
			
			var res15 = decision_first(character15,3,seek15,attack15,wander15,arrive15,evade15,flee15,color_char3,counter6,hammers6)
			counter6 = res15[0]
			hammers6 = res15[1]
			hammers6 = check_hammer_collision(hammers6,character15)
			
			var res16 = decision_first(character16,6,seek16,attack16,wander16,arrive16,evade16,flee16,color_char6,counter7,hammers7)
			counter7 = res16[0]
			hammers7 = res16[1]
			hammers7 = check_hammer_collision(hammers7,character16)
		
		if Show.sound:
			$AudioStreamPlayer.volume_db = -30
		else:
			$AudioStreamPlayer.volume_db = -200
		...
		if createLines:
			if Input.is_action_just_pressed("enter"):
				...
				for child in children:
					if child is Line2D:
						child.visible = true
				var chars =[character,character2,character3,character4,character5,character6,character7,
				character8,character9,character10,character11,character12,character13,character14,character15,
				character16]
				for char in chars:
					char.visible = true
				selected_colors = []
...
func decision_first(char,TYPE,seek,attack,wander,arrive,evade,flee,color,wait : int = 0, hammer : Array = [],char2 : CharacterBody2D = character):
	var decision
	if TYPE == 1:
		var d1 = decision1(char,target)
		decision = d1.makeDecision()
	elif TYPE == 2:
		var d1 = decision2(char,target)
		decision = d1.makeDecision()
	elif TYPE == 3:
		var d1 = decision4(char,target)
		decision = d1.makeDecision()
	elif TYPE == 4:
		var machine = create_state_machine_1(char,target,char2)
		var s = machine.update()
		var priority = 1000
		var found = null
		for x in s:
			if x.priority < priority:
				priority = x.priority
				found = x
		decision = found.value
	elif TYPE == 5:
		var machine = create_state_machine_2(char,target)
		var s = machine.update()
		var priority = 1000
		var found = null
		for x in s:
		...
		var machine = create_state_machine_3(char,target)
		var s = machine.update()
		var priority = 1000
		var found = null
		for x in s:
	...
	if decision == WALK:
		char.modulate = WALK_COLOR
		char.steering = seek.getSteering()
	elif decision == ATTACK:
		char.modulate = ATTACK_COLOR
		char.steering = arrive.getSteering()
	elif decision==GUARD:		
		char.modulate = GUARD_COLOR
		char.velocity = Vector2.ZERO
		char.time=0
	elif decision == EVADE:
		char.modulate = EVADE_COLOR
		char.steering = evade.getSteering()
	elif decision == THROW:
		char.modulate = THROW_COLOR
		char.steering = arrive.getSteering()
		wait+=1
		...
		char.modulate = WANDER_COLOR
		char.steering = wander.getSteering2()
	elif decision == FLEE:
		char.modulate = FLEE_COLOR
		char.steering = flee.getSteering()
...
func create_state_machine_1(char,player,char2):	
...
func create_state_machine_2(char,player):	
...
func create_state_machine_3(char,player):	
	var a1 = MachineAction.new(GUARD,2)
