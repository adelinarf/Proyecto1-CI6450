...
func generate_voronoi_diagram(width, height, num_cells,text,vert):
	#var image = Image.new("RGB", (width, height))
	var dynImage = Image.create(500, 500, false, Image.FORMAT_RGBA8)
	#img.set_pixel(8, 8, Color.RED)
	
	
	#var dynImage = Image.new()
	#dynImage.create_empty(500,500,false,Image.FORMAT_BPTC_RGBA)
...
func orderVertices(p,vertexes):
	#ordenar vertexes en sentido antihorario con respescto a p
	#y una semirrecta de pendiente 0
	var c = []
	center = p
	for v in vertexes:
		for u in v:
...
func visible(p,v,segmentVisible,S):
	...
	#si segmento p v intersecta el segmento visible
	#return false
...
func visibleF(p1,p2,S,node):
	#lado p1,p2 no intersecta con nada en S
	var line1 = p2-p1
	var lines = S 
...
func naiveBetterAlgorithm(point,S,vertexes,l):
	...
	for obstacle in vertexes:
		for vertex in obstacle:
			...
			#for obst in S:
			#	r = min(r, point.distance_to(obst.global_position))
			var x = r*cos(angle)
			var y = r*sin(angle)
