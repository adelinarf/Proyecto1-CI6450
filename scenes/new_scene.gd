...
func getVertexes():
	...
	vertex.append_array([vertex1,vertex2,vertex3,vertex4,vertex5,vertex6,vertex7,vertex8,vertex9])
	return vertex
...
func _ready() -> void:	
	...
	seekc2 = Seek.new(character2,target)
	attack2 = Arrive.new(character2,target)
	pathfollowing = FollowPath.new(character2,target,_tile_pos)
...
func decision_cover():
	var d1 = decision_new2(character2,target)
	var decision = d1.makeDecision()
	if decision != GUARD:
		character2.time=0.1
	if decision == ATTACK:
		...
		print(_tile_pos)
		character2.steering = pathfollowing.getSteeringPrediction2(_tile_pos[0])
		#else:
...
func check_cover_points():
	...
	var cover_nodes = [g1.closest_to_node(Vector2(936,680)),g1.closest_to_node(Vector2(1048,56))] #,g1.closest_to_node(Vector2(160,240)),g1.closest_to_node(Vector2(776,900))]
